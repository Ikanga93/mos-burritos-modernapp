...
export const CartProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (savedCart) {
      try {
        ...
        // FIX: Validate structure before restoration
        if (parsedCart && typeof parsedCart === 'object') {
          // Check if it's the old format (object with items property) or new format (array)
          const itemsToRestore = Array.isArray(parsedCart)
            ? parsedCart
            : (Array.isArray(parsedCart.items) ? parsedCart.items : [])

          if (itemsToRestore.length > 0) {
            restoreCartItemsWithImages(itemsToRestore)
          } else {
            setIsLoaded(true)
          }
        } else {
  ...
  const restoreCartItemsWithImages = async (cartItems) => {
    ...
    try {
      ...
      // FIX: Ensure it's always an array, even on error
